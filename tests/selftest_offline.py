...
def check_scaffold(match, me, extractor, builder, quick_text):
    ...
    for marker in ["## КАК ГОТОВИТЬ РАЗБОР", "ЗАПРЕЩЕНЫ общие советы",
                   # Объективная картина матча стоит ПЕРЕД вердиктом: разбор,
                   # начатый с личных цифр, подгоняет матч под игрока.
                   "### 1. Что произошло в матче", "### 2. Вердикт",
                   "### 4. Главный leak",
                   # Драфт и билд обязательны в любом режиме: по ним нет готовых
                   # чисел, и без раздела разбор сваливается в пересказ статистики.
                   "### 5. Драфт и билд", "Разбирай ДРАФТ, а не только исполнение",
                   "Билд оценивай в КОНТЕКСТЕ",
                   # Причинность: без неё разбор винит игрока в симптомах уже
                   # проигранной карты, и это самый дорогой дефект методики.
                   "ПРИЧИНА ПЕРЕД ВИНОЙ",
                   "Главный leak ищи там, где игра была ЕЩЁ ЖИВОЙ",
                   "Позиционных и посекундных данных в промпте НЕТ",
                   # Линза работает на любой роли, а корень бывает не в игроке.
                   "САМОМУ ЗАМЕТНОМУ плохому показателю",
                   "Корень проблемы может лежать не в игроке",
                   # Тайм-окно как следующий шаг коуч-цикла.
                   "У инструмента есть лупа",
                   "### 8. Гипотезы: почему матч закончился так",
                   "### 9. Вопросы ко мне и следующий шаг"]:
